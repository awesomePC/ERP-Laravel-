...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	...
	<td>
		...
		<div class="input-group input-number">
			...
			<select name="products[{{$row_count}}][sub_unit_id]" class="form-control ml-1 sub_unit">
				@foreach($sub_units as $key => $value)
	...
	<td @if(!$edit_discount) class="hide" @endif>
		<div class="row">	
			<div class="col-md-6">
				{!! Form::text("products[$row_count][line_discount_amount]", @num_format($discount_amount), ['class' => 'form-control input_number row_discount_amount', 'style' => 'width: 100px']); !!}
			</div>
			<div class="col-md-6">
				{!! Form::select("products[$row_count][line_discount_type]", ['fixed' => __('lang_v1.fixed'), 'percentage' => __('lang_v1.percentage')], $discount_type , ['class' => 'form-control row_discount_type', 'style' => 'width: 100px']); !!}
			</div>
		</div>
		@if(!empty($discount))
