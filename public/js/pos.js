$(document).ready(function () {
    ...
    $('button.pos-express-finalize').click(function () {
        ...
        console.log(payment_method_dropdown)
    ...
    //on save card / mfs
    $('button#gt-save-print').click(function (e) {
        e.preventDefault();
        if ($('#gt_mfs_amount').val() != "") {
            var payment_method_dropdown = $('#payment_rows_div')
                .find('.payment_types_dropdown')
                .first();

            payment_method_dropdown.val("card");
            payment_method_dropdown.change();

            $('input#amount_0').val($('#gt_mfs_amount').val());
            $('input#card_number_0').val($('#card_number').val());
            $('input#card_holder_name_0').val($('#card_holder_name').val());
            $('input#card_transaction_number_0').val($('#gt_mfs_card').val());
            $('textarea#note_0').val($('#gt_mfs_card').val());
            $('select#card_type_0').val($('#card_type').val());
            $('input#card_month_0').val($('#card_month').val());
            $('input#card_year_0').val($('#card_year').val());
            $('input#card_security_0').val($('#card_security').val());
    ...
    //Update discount
    $('button#posEditDiscountModalUpdate').change(function () {
    ...
    // GT update discount
    $('#discount_amount_modal').keyup(function () {
...
// change cash_to_return value
let cashReceived = 0;
let cashToReturn = document.querySelector('#cash_to_return');
let cardAmount = document.querySelector('#gt_mfs_amount');

$(document).on('keyup keypress', '#cash_received', (e) => {
    cashReceived = e.target.value;

    // check card amount value
    if (cardAmount.value == '') {
        cardAmount.value = 0;
    }

    let returnAmount = (parseFloat(cashReceived) + parseFloat(cardAmount.value)) - __read_number($('input#final_total_input'));
    cashToReturn.value = returnAmount.toFixed(2);
});

// format card reference field
$(document).on('keyup keypress', '#gt_mfs_card', (e) => {
